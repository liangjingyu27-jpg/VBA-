...
class MainWindow(QMainWindow):
        ...
        self.stage_index["exclude_rules"] = self.stage_stack.addWidget(
            self._wrap_scroll_stage(
                self._build_task_stage(
                    ...
                    stage_note="这里承载当前批次的排除规则候选维护。首轮骨架阶段先用演示任务池占位。",
                )
            )
        )
        self.stage_index["settlement_terms"] = self.stage_stack.addWidget(
            self._wrap_scroll_stage(
                self._build_task_stage(
                    ...
                    stage_note="这里承载当前批次的结算条款待补维护。首轮骨架阶段先用演示任务池占位。",
                )
            )
        )
        self.stage_index["self_pickup"] = self.stage_stack.addWidget(
            self._wrap_scroll_stage(
                self._build_task_stage(
                    ...
                    stage_note="这里承载当前批次的自提待判定维护。首轮骨架阶段先用演示任务池占位。",
                )
            )
        )
        self.stage_index["actual_freight"] = self.stage_stack.addWidget(
            self._wrap_scroll_stage(
                self._build_task_stage(
                    ...
                    stage_note="这里承载当前批次的按实际运费待录入维护。首轮骨架阶段先用演示任务池占位。",
                )
            )
        )
    ...
    def _build_export_stage(self) -> QWidget:
        ...
        self.btn_export_final.clicked.connect(lambda: self._show_feedback("当前状态：首轮骨架阶段暂不执行真实导出写出。"))
        self.btn_export_full.clicked.connect(lambda: self._show_feedback("当前状态：首轮骨架阶段暂不执行真实导出写出。"))
    ...
    def _sheet_has_meaningful_content(self, sheet: Worksheet) -> bool:
        non_empty_count = 0
        for row in sheet.iter_rows(min_row=1, max_row=min(sheet.max_row, 20), max_col=min(sheet.max_column, 12), values_only=True):
            for value in row:
    ...
    def _select_source_sheet(self, workbook: Any) -> tuple[str, Worksheet | None, str]:
        sheets: list[Worksheet] = list(workbook.worksheets)
        if not sheets:
            return "尚未识别", None, "工作簿中未找到任何工作表"

        for sheet in sheets:
            if self._normalize_sheet_name(sheet.title) == "原始数据":
                return sheet.title, sheet, "精确命中《原始数据》"

        for sheet in sheets:
            if "原始数据" in self._normalize_sheet_name(sheet.title):
                return sheet.title, sheet, "名称包含“原始数据”"

        for sheet in sheets:
            if self._sheet_has_meaningful_content(sheet):
                return sheet.title, sheet, "未命中“原始数据”，兜底选择第一个有内容的工作表"

        first_sheet = sheets[0]
        return first_sheet.title, first_sheet, "未命中“原始数据”且未识别到明显内容，兜底选择第一个工作表"

    def _preview_rows_from_sheet(self, sheet: Worksheet, max_rows: int = 5, max_cols: int = 6) -> list[list[str]]:
        preview_rows: list[list[str]] = []
        for row in sheet.iter_rows(min_row=1, max_row=min(sheet.max_row, 12), max_col=min(sheet.max_column, max_cols), values_only=True):
            row_values = [self._safe_text(value) for value in row]
    ...
    def _compute_progress(self) -> int:
        ...
        solved_ratio = 1 - (remaining / total_demo_items if total_demo_items > 0 else 0)
        ...
        if self.current_batch_state in {"待维护", "维护中"}:
            return max(state_base, min(79, int(35 + solved_ratio * 40)))
        return state_base
    ...
    def _handle_import_file(self) -> None:
        ...
        try:
            ...
            self.available_sheet_names = workbook.sheetnames

            selected_name, selected_sheet, reason = self._select_source_sheet(workbook)
            self.current_source_sheet = selected_name
            self.current_source_reason = reason

            if selected_sheet is not None:
                self.source_rows = int(selected_sheet.max_row or 0)
                self.source_cols = int(selected_sheet.max_column or 0)
                self.source_preview_rows = self._preview_rows_from_sheet(selected_sheet)
            else:
                self.source_rows = 0
                self.source_cols = 0
                self.source_preview_rows = []
            ...
            self.gate_state = "未通过"
            self.last_run_summary = "首轮骨架阶段：已基于导入文件加载演示任务池，用于验证新骨架。"
            self.current_stage_key = "overview"
            self._fill_sheet_preview_table()
            self._refresh_ui()
            self._scroll_current_stage_to_top()
            self._show_feedback(
                f"最近一次运行：已导入《{self.current_file_name}》，当前源表为《{self.current_source_sheet}》，并已加载演示任务池。"
            )
    ...
    def _clear_task_category(self, key: str) -> None:
        ...
        if remaining > 0:
            ...
            self.last_run_summary = f"演示保存：{self.task_meta[key]['title']} 已清零，当前批次仍有其他待处理任务。"
            self._show_feedback(f"当前提示：{self.task_meta[key]['title']} 已清零，建议继续处理剩余分类。")
        else:
            self.current_batch_state = "可再运行"
            self.last_run_summary = "演示保存：当前维护项已清零，建议再次运行刷新批次状态。"
    ...
    def _refresh_ui(self) -> None:
        ...
        self.overview_task_pool_summary.set_value(self._task_pool_summary_text())
